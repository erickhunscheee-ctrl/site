  ...
  return (
    <div className="border-l-4 border-[#ca1515] hover:border-l-[#cfc8c8] bg-[#151313] rounded-s-[20px] p-8 hover:bg-[#1a1818] transition-colors group">
      <div className="flex flex-col items-center text-center mb-6">
        ...
        <h3 className="text-white text-lg md:text-xl font-semibold mb-2">Whatsapp</h3>
        <p className="text-gray-300 text-sm leading-relaxed mb-6">
          Envie, receba e automatize mensagens diretamente pelo WhatsApp usando nossa API integrada.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-3 bg-[#262A2C] border border-transparent rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-[#25D366] transition-colors"
            disabled={isLoading}
          />
        </div>
        ...
        {status === "success" && (
          <div className="text-center text-green-400 text-sm">Para enviar mensagens automaticas, entre em contato conosco!</div>
        )}

        {status === "error" && (
          <div className="text-center text-red-400 text-sm">Para enviar mensagens automaticas, entre em contato conosco.</div>
        )}
      </form>
    </div>
  )
